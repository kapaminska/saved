...
- FR-001: User can log in via magic link (email-only). Priority: must-have
  > Socrates: Counter-argument: magic link is simpler than OAuth. Resolution: revised from OAuth to magic link — simpler, no third-party dependency.
...
- FR-007: User can abandon a goal (status changes to abandoned), preserving its data. Priority: must-have
...
- FR-022: User can permanently delete a payment. Priority: must-have
  > Socrates: No counter-argument; stands as written. Payments are atoms — delete + recreate is the fix for misassignment.

### Net Worth Panel

- FR-023: User can add, edit, and remove assets (name, amount, category from closed list: cash / savings / investments / real estate / other). Priority: nice-to-have
...
- FR-024: User can add, edit, and remove liabilities (name, amount). Priority: nice-to-have
...
- FR-025: App displays net worth (sum of assets minus liabilities). Priority: nice-to-have
  > Socrates: Counter-argument: negative net worth is demoralizing. Resolution: kept — honest math, no sugarcoating.
- FR-026: User can confirm an asset's current value (refreshes its last update date without changing amount). Priority: nice-to-have
...
- FR-027: App shows a soft prompt when any asset has not been updated in over 3 months. Priority: nice-to-have
...
- FR-031: App creates a new user account on first magic-link login. Priority: must-have
  > Socrates: No counter-argument; stands as written.
...
1. **Natural-language check-in parsing** — przyjmuje zdanie użytkownika (np. „500 na wakacje, 1000 na poduszkę"), rozpoznaje kwoty i nazwy celów, zwraca strukturalne wpłaty z przypisaniem do istniejących celów. Nierozpoznane nazwy są flagowane.
...
5. **Monitorowanie wieku aktywów** — jeśli najstarsze aktywo nie było aktualizowane od ponad 3 miesięcy, jeden banner z propozycją aktualizacji. Dismissable.

## Access Control

Magic link (email-only) — bez OAuth, bez hasła. Flat user model — każdy zalogowany użytkownik widzi wyłącznie swoje dane. Brak ról (admin, member, guest). Flag „w związku małżeńskim" wpływa tylko na etykiety w UI („Twoja" vs „Wasza" wartość netto), nie na kontrolę dostępu. Niezalogowany użytkownik widzi mini-landing z przyciskiem logowania.
...
- Import danych z banków — brak integracji bankowych, skanowania paragonów, automatycznego importu transakcji.
...
- Permanent deletion of goals — goals are only abandoned, never destroyed.
